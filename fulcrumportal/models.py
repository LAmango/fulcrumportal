...
class Requests(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    request_fullname = db.Column(db.String(40), nullable=False)
    user_business = db.Column(db.String(80), db.ForeignKey('user.business_name'), nullable=False)
    date_posted = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)
    request_body = db.Column(db.String(250), nullable=False)

    def __repr__(self):
        return f"Request('{self.request_fullname}', '{self.user_business}')"